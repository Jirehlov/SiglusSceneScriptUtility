...
- Fixed Python original-source embedding to include dot-prefixed `.ss` and `.inc` files, matching the Rust backend and preserving them for extraction and recompilation.
- Fixed LSP definition and reference lookup for label targets in `gosub` and `gosubstr` expressions. Older cached indexes are rebuilt to apply the fix.
