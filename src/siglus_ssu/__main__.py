...
def _usage(out=None):
    ...
    text = (
        ...
        "    --apply        For global.sav: apply same-directory global.txt to G/Z/cg_table/bgm_table/chrkoe\n"
        ...
        f"  {p} -m --dbs <game_root>\n"
        f"  {p} -m --dbs-undo <game_root>\n"
        f"  {p} -m --disam <path_to_dat|path_to_dir>\n"
        f"  {p} -m --disam-apply <path_to_dat|path_to_dir>\n"
        "    --apply        Apply .ss CSV back to .ss\n"
        "    --dbs          Build repeatable dynamic .dbs text loading in-place\n"
        "    --dbs-undo     Restore files changed by --dbs from the manifest\n"
