...
def analyze_gan(path, blob):
    ...
    if sets:
        ...
        for i, s in enumerate(sets):
            ...
            for j, p in enumerate(pats[:MAX_LIST_PREVIEW]):
                print(
            ...
            if len(pats) > MAX_LIST_PREVIEW:
                print("  ... (%d patterns omitted)" % (len(pats) - MAX_LIST_PREVIEW))
        print("")
...
def compare_files(p1, p2):
    ...
    if t1 != t2:
        ...
        return 0
    if t1 == "gan":
        return compare_gan(p1, p2, b1, b2)
    if t1 == "dbs":
