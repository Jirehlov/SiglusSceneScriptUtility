...
| `--payload` | **（仅比较模式）** 对 `.pck` 和 `.dat` 额外比较解码/解压后的场景 payload，覆盖全部字节码操作数、标签表、场景属性布局、完整字符串池（包括未引用的字符串）、已读标志表，以及包级属性和命令路由元数据。`same` 忽略场景字符串池存放顺序、相应字符串索引的洗牌，以及仅供调试使用的 `namae_list`；`text_only` 表示非字符串结构相同、只有解析后的字符串内容不同，用于识别翻译修改；其余差异为 `real_diff`。内部标签重编号、参数表编号、其他表项的数量和顺序变化均保留；标识符名称表保留索引偏移和长度、空项、NUL 和原始 UTF-16 码元，这些表的变化归为 `real_diff`。姓名表原始内容仍写入反汇编并检查索引越界，但不参与 payload 哈希：官方引擎将该表用于调试器的姓名数据库。字符串也可能是资源名或场景名，因此 `text_only` 不保证运行行为不变。`.pck` 的 payload 栏报告有字节差异的场景，结果为 `same`、`text_only`、`real_diff` 或无法解析时的 `-`；`.dat` 对应使用 `identical`、`text_only`、`real_diff` 或 `unavailable`。字节码截断、缺少末尾 `CD_EOF`、`CD_EOF` 后仍有字节码、负参数数量或字符串索引越界均报告 `INCOMPLETE` 并返回失败。加密场景无法用可用 key 解码时，payload 比较失败。Rust 原生 payload scanner 可用时自动使用 Rust，否则回退到 Python。 |
