...
普通文本展开路径保留“剩余文本长度连续超过 10000 步未降到新低则报错”的保护。宏内部的处理因 backend 而异，见下节；不能依赖该外层保护中止宏内部的循环。
...
**循环展开与警告：** 避免在宏实参、默认文本或宏体中触发 `#define` / `#define_s` 的自引用或相互引用循环。Python 前端不会因为宏内部的替换次数多而强制停止，因此真正的无限展开仍可能使编译或 LSP 分析一直运行。

每段宏实参（含默认值）或宏体展开的替换计数达到 **10000** 时，Python 前端会向 `stderr` 输出一次警告，随后继续展开，不截断结果。文本发生改变或扫描位置不前进的步骤计入该计数，单纯字符扫描不计入。该提醒不是无限循环判定：有限的长展开仍可完成。它适用于 Python 编译 backend（包括 `--legacy`、`--legacy-full` 和纯 Python 安装）及 LSP 的 Python 分析路径；普通展开和 Rust 原生 backend 既有的超限报错行为不变。若确认存在循环，请中止编译进程（终端可用 `Ctrl+C`）并修改定义；LSP 卡住时应修正文档并重启服务。
