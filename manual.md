...
- Cyclic `#define` / `#define_s` expansion in macro arguments, defaults, or bodies can leave analysis running indefinitely, preventing diagnostics and subsequent requests from completing. Both frontends write their 10000-step warning only to `stderr` and do not stop expansion. Correct the document and restart the language server if it hangs; see [macro expansion](#macro).
...
| `--allow-invalid` | Replace invalid byte sequences with U+FFFD when decoding compile inputs, including `Gameexe.ini` and `暗号.dat`, using the encoding selected by `--charset`. Without `--charset`, keep strict UTF-8/CP932 detection and use UTF-8 replacement only if both fail. Defaults to off; syntax and type errors still fail compilation. |
...
Detailed project-wide statistics are omitted instead of printed as `n/a` when the run is not a normal full scene compilation, including `--tmp`, `--dat-repack`, `--test-shuffle`, `--gei`, and partial or failed compile runs. The basic timing and file-count summary remains available when the corresponding stages ran.
...
- **Source decoding:** If `--charset` is not specified, each source file is decoded independently by testing UTF-8 and CP932, using the BOM, strict decode results, and deterministic ambiguity scoring to choose between them. Other Python codecs must be selected explicitly. Decoding is strict by default; invalid byte sequences before DOS EOF fail the compile. `--allow-invalid` enables replacement with U+FFFD. To reproduce historical output built by reading CP932 bytes as UTF-8, use `--charset utf8 --allow-invalid`; this intentionally preserves the resulting garbled strings instead of detecting CP932. Decoding removes one leading Unicode BOM and normalizes CRLF or lone CR to LF, but performs no Unicode normalization. Both compiler backends receive the same decoded text snapshot.
- **DOS EOF:** Compilation stops reading each source at the first U+001A character in its encoding and ignores the remaining bytes, even if they are malformed. A `0x1A` byte inside another UTF-16/UTF-32 character is not EOF. Embedded original sources and source hashes retain the complete file bytes; ordinary non-compile text reads are unchanged.
...
- **Key strings and cross-platform consistency:** When deriving a key from `暗号.dat` or `angou=text`, both backends use Python CP932 encoding on every platform and ignore unencodable characters. These rules do not depend on the Windows version or system code page. The official compiler uses Windows `WideCharToMultiByte(CP_ACP, 0, ...)`, so the same string does not always produce the same key. For example, under Japanese CP932, SSU encodes `髙` as `EE E0`, while Windows uses `FB FC`; SSU ignores `¥`, while Windows converts it to `5C`. These differences can prevent correct decryption of `Scene.pck` or `Gameexe.dat` and can also change whether the encoded string meets the 8-byte minimum for enabling encryption. To interoperate with official output, use the raw 16-byte key from the matching engine: for compilation, supply `key.txt` and ensure the input directory has no higher-priority `暗号.dat`; for analysis, extraction, and other operations supporting `--angou`, supply `key=bytes` or the matching `SiglusEngine.exe` directly.
- **Incremental compilation:** When `--tmp` is specified, the compiler caches SHA-256 hashes of all `.ss` and `.inc` files. Each hash is calculated from the same raw bytes used to decode that file for compilation; a file saved after its snapshot was read is detected as changed on the next run. Cache compatibility includes the `siglus-ssu` version, source charset, the `--allow-invalid` setting, active `const.py` content/profile, and scene string XOR multiplier. On the next compatible run, only files whose hash has changed (or whose `.dat` is missing) are recompiled, and existing `.lzss` outputs are reused. If a scene source changes or its `.lzss` is missing, that scene's `.lzss` is regenerated. Case-only source renames also remove conflicting old `.dat` files when recompilation is needed and invalidate the matching `.lzss` regardless of its filename case. If any `.inc` file is added, changed, or removed, or cache metadata is incompatible, a full recompile is triggered.
...
```
# Analyze a .pck file
siglus-ssu -a <input_file.pck> [--angou <path|angou=text|key=bytes>]

# Analyze a .dat file
siglus-ssu -a [--disam] <input_file.dat> [--angou <path|angou=text|key=bytes>]
...
# Compare two .pck files
siglus-ssu -a [--payload] <input_file_1.pck> <input_file_2.pck> [--angou <path|angou=text|key=bytes>]

# Compare two .dat files
siglus-ssu -a [--payload] [--disam] <input_file_1.dat> <input_file_2.dat> [--angou <path|angou=text|key=bytes>]
...
| `--disam` | When analyzing a `.dat` file or comparing two `.dat` files, write human-readable disassembly to `<scene>.dat.txt` alongside each input `.dat`. Prints total disassembly timing before the command finishes. Use `-x --decompile <input_pck>` for reconstructed `.ss` output. |
...
| `--payload` | **(Compare mode only)** Additionally compare decoded/decompressed scene payloads for `.pck` and `.dat`, including every bytecode operand, label tables, scene property layouts, the complete string pool (including unreferenced strings), read-flag tables, and pack-level property/command routing metadata. `same` ignores scene string-pool storage order, the corresponding string-index shuffle, unused trailing zero entries in the Z-label table, and the debug-only `namae_list`; `text_only` means unchanged non-string structure with changes only to resolved string contents, for identifying translation edits; other differences are `real_diff`. Z-label numbers, interior empty entries, and nonzero jump offsets remain part of the comparison. Internal label renumbering, argument-list IDs, and other table entry counts and ordering are preserved. Scene identifier name tables retain index offsets and lengths, empty entries, NULs, and the original UTF-16 code units; changes to these tables are classified as `real_diff`. Pack-level names also preserve empty entries, NULs, and the original UTF-16 code units. Raw `namae_list` entries remain in disassembly and are checked for out-of-range indices, but do not enter payload hashes: the official engine uses this table for its debugger name database. Strings can also be resource or scene names, so `text_only` does not guarantee unchanged runtime behavior. The `.pck` payload column reports scenes with byte differences as `same`, `text_only`, `real_diff`, or `-` when parsing is unavailable; `.dat` uses `identical`, `text_only`, `real_diff`, or `unavailable`, respectively. Truncated bytecode, a missing final `CD_EOF`, bytecode after `CD_EOF`, negative argument counts, out-of-range string indices, undefined branch labels, or branch targets that do not point to instruction starts produce `INCOMPLETE` and return failure. Payload comparison fails when encrypted scenes cannot be decoded with an available key source. The Rust native payload scanner is used when available, with Python as the fallback. |
...
# Compare two Scene.pck files by normalized scene runtime semantics
...
Strings in `m_type=0` use fixed Windows CP932 encoding and decoding on every platform, including extension characters such as `①` and `髙`, independently of the system code page. When compiling, every string must encode strictly as CP932; other types use strict UTF-16LE encoding. An unencodable cell reports its CSV path, record start line, column, and call number, and no invalid `.dbs` is written.
...
| `--m` | **Merge** mode. Composites multiple `.g00` images or cuts into one PNG while preserving straight-alpha RGB and aligning type2 layers by the selected cut centers. With `--trim`, crops transparent or opaque-background edges from the merged PNG. |
...
| Parameter | Description |
|---|---|
| `<input_pck>` | Input `Scene.pck` archive to analyze. Encrypted scene data requires a valid key source; otherwise the command fails without writing JSON or a viewer. |
...
3. recompiles the extracted source in place, trying `const-profile` 0 through 9 in order until one profile produces `EXACT` or `PAYLOAD_SAME`; if all profiles fail, retries the same profiles in order with `--charset utf8 --allow-invalid`; each attempt uses that profile's compiler rules and default string multiplier unless the multiplier was explicitly overridden;
...
Step log lines report status only. A completed file is `EXACT` when the original and rebuilt `.pck` bytes match exactly, or `PAYLOAD_SAME` when the bytes differ but the normalized payload comparison succeeds. `PASS` is not a result category. The `total` line and final summary include timings for `analyze`, `extract`, `compile`, `payload`, and `cleanup` when those steps run. The `compile` and `payload` timings include all attempts across both rounds. Logs identify attempts using `--charset utf8 --allow-invalid`; this retry does not relax the comparison criteria.

Compile errors are kept quiet while attempts remain in either round. If neither round produces a matching rebuild, the file is marked as `FAIL`: the command reports the closest payload comparison, or the captured compile output if no attempt reached comparison.
...
`-c` first decodes each source file up to its first U+001A (DOS EOF), according to the source encoding selected by `--charset` or auto-detected by the compiler. Decoding is strict unless `--allow-invalid` is enabled, which replaces invalid byte sequences with U+FFFD; when neither UTF-8 nor CP932 passes strict auto-detection, this option uses UTF-8 replacement. Bytes after DOS EOF are ignored for compilation. One leading Unicode BOM is removed, CRLF and lone CR line endings are normalized to `\n`, and no NFC/NFKC or other Unicode normalization is performed. The CA stage also removes any remaining `\r` before character-level processing. The remaining rules of the SiglusSS language are therefore defined over this normalized LF text stream and do not depend on the original byte encoding.
...
Ordinary text expansion retains a guard that reports an error after more than 10000 consecutive steps without a new minimum remaining-text length. Both backends warn and continue inside macros, as described below; the outer guard cannot be relied on to stop a loop inside a macro.
...
**Expansion loops and warnings:** Avoid triggering self-referential or mutually recursive `#define` / `#define_s` expansion inside macro arguments, defaults, or bodies. Neither frontend stops macro expansion solely because of its replacement count, so a genuine infinite expansion can leave compilation or LSP analysis running indefinitely.

For each macro argument (including a default) or body expansion, both frontends write one warning to `stderr` when the replacement counter reaches **10000**, then continue without truncating the result. Steps that change the text or leave the scan position unchanged count toward this threshold; ordinary character scanning does not. This is not proof of an infinite loop: a long but finite expansion can still complete. The warning applies to both compile backends, including `--legacy`, `--legacy-full`, and pure-Python installations, and both LSP analysis paths. Existing threshold errors in ordinary expansion are unchanged. If a cycle is confirmed, stop compilation (`Ctrl+C` in a terminal) and fix the definitions; if LSP hangs, correct the document and restart the server.
