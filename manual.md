...
```
siglus-ssu [-h] [-V | --version] [--legacy] <mode> [args]
```
...
```bash
# Basic initialization (downloads const.py from the main branch)
siglus-ssu init
...
| `--dat-repack` | Instead of compiling `.ss` scripts, scan `input_dir` for existing `.dat` files, copy them, and pack them directly into a `.pck` file. Useful for packing already-compiled scripts. Compatible only with `--no-os`. Cannot be combined with `--test-shuffle`. |
...
| `<output_dir>` | Directory where extracted files will be written. Required for extracting `.pck`, but optional for `--gei` (defaults to the input directory). |
| `--disam` | In addition to extracting `.ss` source files, also dump a human-readable disassembly of each compiled `.dat` file into a `__DATDIR__` subdirectory. Cannot be combined with `--gei`. |
...
# Extract with .dat disassembly (creates a __DATDIR__ subfolder)
...
|---|---|
| `.nwa` | NeWA Adaptive differential PCM compressed audio. Decodes to `.wav`. |
